  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" width="250" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="success" size="mini" @click="handleInspect(scope.row)">查看</el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <!--<el-form-item label="用户名" prop="nickname" hidden="hidden">
          <el-input v-model="dataForm.nickname"/>
        </el-form-item>
        <el-form-item label="手机号码" prop="mobile" hidden="hidden">
          <el-input v-model="dataForm.mobile"/>
        </el-form-item>
        <el-form-item label="密码" prop="password" hidden="hidden">
          <el-input v-model="dataForm.password" type="password" auto-complete="off"/>
        </el-form-item>
        <el-form-item label="性别" prop="gender" hidden="hidden">
        ...
        <el-form-item label="生日" prop="birthday" hidden="hidden">
    ...
    <el-dialog :visible.sync="dialogInspect" title="查看">
      <div>总支出：300</div>
      <el-collapse v-model="activeNames" @change="handleChange">
        <el-collapse-item title="用户使用记录" name="1">
          <div>与现实生活一致：与现实生活的流程、逻辑保持一致，遵循用户习惯的语言和概念；</div>
          <div>在界面中一致：所有的元素和结构需保持一致，比如：设计样式、图标和文本、元素的位置等。</div>
        </el-collapse-item>
        <el-collapse-item title="资金流动统计" name="2">
