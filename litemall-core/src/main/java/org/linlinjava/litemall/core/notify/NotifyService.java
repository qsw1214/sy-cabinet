...
import org.linlinjava.litemall.core.notify.config.NotifyProperties.sign;
...
public class NotifyService {
	...
	private List<sign> Alisign = new ArrayList<>();
	...
	public void notifySmsTemplate(String phoneNumber, NotifyType notifyType, String[] params) {
		...
		SmsResult sms = smsSender.sendWithTemplate(phoneNumber, templateId, params);
	 ...
	 */
	@Async
	public void AliSms(String phoneNumber, NotifyType notifyType, String params) {

		for (sign item : Alisign) {
			String notifyTypeStr = notifyType.getType();
			if (item.getKey().equals(notifyTypeStr)) {
				AlibabaSmsSender aliSms = new AlibabaSmsSender();
				SmsResult sms = aliSms.sendWithTemplate(phoneNumber, item.getName(), item.getTemplatecode(),
						params,aliappid,aliappkey);
				System.out.println(sms.getResult());
			}
		}
	...
	public List<sign> getAlisign() {
		return Alisign;
	}

	public void setAlisign(List<sign> alisign) {
		Alisign = alisign;
	}
