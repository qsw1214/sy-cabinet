...
public class AdminOrderService {
	...
	public Object list(String mcnum,String region, Integer page,Integer limit) {
		List<LitemallMainCabinet> MainCabinet = mainCabinetService.querySelective(mcnum,region, page, limit);
		long total = PageInfo.of(MainCabinet).getTotal();
		Map<String, Object> data = new HashMap<>();
		data.put("total", total);
	 ...
	 * 分页查询子柜信息
	 */
	public Object listSub(Integer mainId,String mainCabNum,String subCabNum,Integer status, Integer standardId, Integer page,Integer limit){
		//查询子柜信息
		List<LitemallSubCabinet> subCabinetList = subCabinetService.listSubCabInfo(mainId,mainCabNum, subCabNum, status, standardId, page, limit);
		//子柜信息数量
		long total = PageInfo.of(subCabinetList).getTotal();
		Map<String, Object> subCabMap = new HashMap<>();
		subCabMap.put("total",total);
		subCabMap.put("items", subCabinetList);
		//返回查询状态
		return ResponseUtil.ok(subCabMap);
	}
	 ...
	 *
	 */
	public Object listStan(String stanName, Integer stanMainId, Integer page,Integer size){
		//查询规格信息
		List<LitemallStandards> standardsList = standardsService.listStandard(stanName, stanMainId, page, size);
		long total = PageInfo.of(standardsList).getTotal();
		Map<String, Object> stanMap = new HashMap<>();
		stanMap.put("total",total);
		stanMap.put("items", standardsList);
		//返回查询状态
		return ResponseUtil.ok(stanMap);
	}
	 ...
	 * @return 订单操作结果 成功则 { errno: 0, errmsg: '成功' } 失败则 { errno: XXX, errmsg:
	 *         XXX }
	 */
