...
public class AdminUserController {
    ...
    public Object list(String nickname, String mobile,
        ...
        List<LitemallUser> userList = userService.querySelective(nickname, mobile, page, limit, sort, order);
    ...
    public Object update(@RequestBody LitemallUser user) {
        ...
        //抛弃原来老方法，因为小程序只是微信登陆，所以管理员不用修改用户的其他信息，只需要修改用户是否可用以及用户的会员等级
        LitemallUser newuser = new LitemallUser();
        newuser.setUserLevel(user.getUserLevel());
        newuser.setStatus(user.getStatus());
        newuser.setId(user.getId());
        if (userService.updateById(newuser) == 0) {
